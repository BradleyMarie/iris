...
TEST(Uber, Empty) {
  ...
  std::shared_ptr<NestedMaterialBuilder> result0 = BuildObject(
      *g_uber_builder, tokenizer, std::filesystem::current_path(),
...
TEST(Uber, WithDefaults) {
  std::stringstream input(
      "\"float Kr\" 1.0 \"float Kt\" 1.0 \"float Kd\" 1.0 \"float Ks\" 1.0 "
      "\"float roughness\" 0.5 \"float uroughness\" 0.5 \"float vroughness\" "
      "0.5 \"float eta\" 1.5 \"float opacity\" 1.0 \"bool remaproughness\" "
...
TEST(Uber, OverridesDefaults) {
  ...
  TextureManager texture_manager;
  std::shared_ptr<NestedMaterialBuilder> result0 = BuildObject(
      *g_uber_builder, tokenizer0, std::filesystem::current_path(),
      spectrum_manager, texture_manager, g_material_manager, texture_manager);
  EXPECT_TRUE(result0);

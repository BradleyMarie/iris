...
ReferenceCounted<Geometry> MakeZeroBoundsGeometry() {
  ...
  EXPECT_CALL(*result, ComputeBounds(_))
...
TEST(SceneObjects, BuildWithDeduplication) {
  ...
  SceneObjects scene_objects = builder.Build(true);
  ...
  scene_objects = builder.Build(true);
...
TEST(SceneObjects, BuildNoDeduplication) {
  ReferenceCounted<Geometry> geom0 = MakeGeometry(false);
  ReferenceCounted<Geometry> geom1 = MakeGeometry(false);
  ReferenceCounted<Geometry> geom2 = MakeGeometry(false);
  ReferenceCounted<Geometry> geom3 = MakeGeometry(true);
  ReferenceCounted<Geometry> geom4 = MakeZeroBoundsGeometry();
  ...
  SceneObjects scene_objects = builder.Build(false);
  ASSERT_EQ(8u, scene_objects.NumGeometry());
  EXPECT_EQ(geom0.Get(), &scene_objects.GetGeometry(0).first);
  EXPECT_EQ(matrix1, *scene_objects.GetGeometry(0).second);
  EXPECT_EQ(geom0.Get(), &scene_objects.GetGeometry(1).first);
  EXPECT_EQ(matrix1, *scene_objects.GetGeometry(1).second);
  EXPECT_EQ(geom1.Get(), &scene_objects.GetGeometry(2).first);
  EXPECT_EQ(matrix2, *scene_objects.GetGeometry(2).second);
  EXPECT_EQ(geom1.Get(), &scene_objects.GetGeometry(3).first);
  EXPECT_EQ(matrix2, *scene_objects.GetGeometry(3).second);
  EXPECT_EQ(geom2.Get(), &scene_objects.GetGeometry(4).first);
  EXPECT_EQ(matrix2, *scene_objects.GetGeometry(4).second);
  EXPECT_EQ(geom2.Get(), &scene_objects.GetGeometry(5).first);
  EXPECT_EQ(matrix2, *scene_objects.GetGeometry(5).second);
  EXPECT_EQ(geom3.Get(), &scene_objects.GetGeometry(6).first);
  EXPECT_EQ(nullptr, scene_objects.GetGeometry(6).second);
  EXPECT_EQ(geom3.Get(), &scene_objects.GetGeometry(7).first);
  EXPECT_EQ(nullptr, scene_objects.GetGeometry(7).second);
  std::set<const Light*> built_lights = GetLights(scene_objects);
  ASSERT_EQ(5u, built_lights.size());
  built_lights.erase(light0.Get());
  built_lights.erase(light1.Get());

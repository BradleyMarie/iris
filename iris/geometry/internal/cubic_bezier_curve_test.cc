...
TEST(CubicBezierCurve, MaxHalfWidth) {
  Point points[4] = {Point(0.0, 0.0, 0.0), Point(1.0, 1.0, 0.0),
                     Point(2.0, 0.0, 0.0), Point(3.0, 1.0, 0.0)};

  EXPECT_EQ(1.0, CubicBezierCurve(points, 0.0, 1.0).MaxHalfWidth());
  EXPECT_EQ(2.0, CubicBezierCurve(points, 2.0, 1.0).MaxHalfWidth());
}

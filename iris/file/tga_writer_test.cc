...
TEST(TgaWriterTest, Black) {
  Framebuffer framebuffer(std::make_pair(5, 7));

  std::stringstream output;
  EXPECT_TRUE(WriteTga(framebuffer, Color::CIE_XYZ, output));

  std::ifstream reference = RunfilePath("black.tga");
...
TEST(TgaWriterTest, White) {
  ...
  std::ifstream reference = RunfilePath("white.tga");
  std::ostringstream reference_string;
  reference_string << reference.rdbuf();
